...
{
    public enum WorldState{ Grounded, InAir }
    ...
    IEnumerator JumpUp(float UpwardsAmt)
    {
        ...
        if (UpwardsAmt != 0)
        {
            rb.AddForce((transform.up * UpwardsAmt), ForceMode.Impulse);
        }
    ...
    void MoveSelf(float d, float Speed, float Accel)
    {
        ...
        if (!MoveInput) //if we are not pressing a move input we move towards velocity //or are crouching
        {
            ...
            MovDirection = Vector3.Lerp(transform.forward, rb.velocity.normalized, 12f * d);
        }
        else
        {
